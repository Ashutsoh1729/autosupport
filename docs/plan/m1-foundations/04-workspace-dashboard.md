...
## Description
Auto-create a default workspace (with an owner membership) when a user signs up, and build the protected dashboard page that shows the current user's workspace. Deploy to Vercel and verify the milestone is demoable: **sign up and see a workspace**.
...
## Workflow
Add a Better Auth `databaseHooks` hook that creates workspace + membership in a Drizzle transaction → build `/dashboard` as a server component that reads the session via `auth.api.getSession`, queries the user's workspace via Drizzle, and renders it with a sign-out button → add a shared header → build → commit → deploy → verify the sign-up → dashboard flow in production.
...
- [ ] Add a fallback message if the user somehow has no workspace
...
| `DashboardPage` | `src/app/dashboard/page.tsx` | Reads session + workspace, renders dashboard |
| `SignOutButton` | `src/components/sign-out-button.tsx` | Calls `authClient.signOut` |

## Data Model
- Writes `workspaces` + `memberships` rows in a transaction (user is created by Better Auth).
- Reads the current user's `workspaces` row via `memberships`.

## Boundaries
- External: `better-auth` (`auth`, `authClient`), Drizzle, Vercel.
- Dashboard reads data directly via Drizzle (server component); REST endpoints for workspaces are not needed in M1.
- Session comes from `auth.api.getSession`; no `src/actions/register.ts` exists (sign-up is handled by Better Auth).

## Considerations
- Guard against a user with no workspace — show a fallback instead of crashing.
